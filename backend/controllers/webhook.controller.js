...
const reserveInboundMessage = async ({ mobile, message, text, attachment }) => {
    ...
    const conversation = await Conversation.findOneAndUpdate(
        { phoneNumber: mobile, status: 'active', 'messages.metadata.waId': { $ne: message.id } },
        {
    ...
    const reserved = conversation.messages.some(item => item.metadata?.waId === message.id);
    return { duplicate: !reserved, conversation };
};
...
const processInboundMessage = async (message, value) => {
    ...
    try {
        ...
        if (message.id) await Conversation.updateOne({ _id: conversation._id }, { $pull: { messages: { 'metadata.waId': message.id } }, $inc: { 'metadata.unreadCount': -1 } });
        throw error;
    }
};
