...
async function test() {
    ...
    const eligibleContacts = await Contact.find({ $or: conditions }).select('_id name professionCategory professionSubCategory company fullName');
    console.log(`Found ${eligibleContacts.length} eligible contacts in DB.`);

    const eligibleIds = eligibleContacts.map(c => c._id);
    console.log("Eligible IDs:", eligibleIds);
