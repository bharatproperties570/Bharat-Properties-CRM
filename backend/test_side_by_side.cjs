...
async function run() {
    ...
    console.time("Driver Find");
    await db.collection('deals').find(query).sort({ updatedAt: -1 }).skip(0).limit(20).toArray();
    console.timeEnd("Driver Find");
    
    console.time("Mongoose Find");
    await Deal.find(query).sort({ updatedAt: -1 }).skip(0).limit(20).lean();
    console.timeEnd("Mongoose Find");
