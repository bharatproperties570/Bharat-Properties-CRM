...
# We will read stage_page_edits.txt which contains JSONL lines of transcript
transcript_file = "stage_page_edits.txt"
base_file = "old_stage_page.jsx"
output_file = "src/pages/Settings/views/StagePage.jsx"

with open(base_file, 'r') as f:
    current_content = f.read()

# We need to find every tool call in the transcript that targeted StagePage.jsx
with open(transcript_file, 'r') as f:
    for line in f:
        try:
            data = json.loads(line.strip())
            # Check if this is a tool_calls line
            if 'tool_calls' in data:
                for call in data['tool_calls']:
                    if call['name'] in ['replace_file_content', 'multi_replace_file_content']:
                        args = call['args']
                        # Sometimes args is a JSON string, sometimes a dict
                        if isinstance(args, str):
                            args = json.loads(args)
                        
                        target_file = args.get('TargetFile', '')
                        if 'StagePage.jsx' in target_file:
                            # Apply the changes
...
# Now apply my manual Engine Status tab patch
status_tab_content = """
            {/* ─── TAB 3: Engine Status ─── */}
            {activeTab === 'status' && (
...
if "{/* TAB 3: Lead ↔ Deal Sync Engine */}" in current_content:
    current_content = current_content.replace("{/* TAB 3: Lead ↔ Deal Sync Engine */}", status_tab_content + "\n            {/* TAB 4: Lead ↔ Deal Sync Engine */}")

with open(output_file, 'w') as f:
    f.write(current_content)

print(f"File restored. Length: {len(current_content.splitlines())} lines")
