        ...
        <>
            ...
            <div className="glass-card" style={{ borderRadius: '16px', border: '1px solid rgba(79, 70, 229, 0.3)', boxShadow: '0 8px 32px 0 rgba(79, 70, 229, 0.08)' }}>
                <div onClick={() => toggleSection('probability')} style={{ padding: '14px 20px', background: 'rgba(79, 70, 229, 0.05)', borderBottom: '1px solid rgba(79, 70, 229, 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
                    <span style={{ fontSize: '0.75rem', fontWeight: 900, color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', textTransform: 'uppercase', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <i className="fas fa-chart-line"></i> AI Closing Probability
                    </span>
                    <i className={`fas fa-chevron-${expandedSections.includes('probability') ? 'up' : 'down'}`} style={{ fontSize: '0.8rem', color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)' }}></i>
                </div>
                {expandedSections.includes('probability') && (
                    <div style={{ padding: '20px' }}>
                        <div style={{ marginBottom: '24px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '12px' }}>
                                <div style={{ fontSize: '1.8rem', fontWeight: 900, color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', letterSpacing: '-1px' }}>{aiStats.closingProbability.current}%</div>
                                <div style={{ fontSize: '0.65rem', color: isDark ? 'var(--text-muted)' : '#64748b', fontWeight: 800, background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#f1f5f9', padding: '2px 8px', borderRadius: '6px' }}>{aiStats.closingProbability.history}</div>
                            </div>
                            <div style={{ height: '10px', background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#f1f5f9', borderRadius: '5px', overflow: 'hidden', display: 'flex', border: '1px solid rgba(0,0,0,0.03)' }}>
                            ...
                            {aiStats.closingProbability.stages.map((st, i) => (
                                <div key={i} style={{ position: 'relative', marginBottom: '16px' }}>
                                    <div style={{
                                        ...
                                        height: '10px',
                                        borderRadius: '50%',
                                        background: dealStatus === 'lost' && st.status === 'active' ? '#ef4444' : st.status === 'completed' ? isDark ? 'var(--bg-card)' : 'var(--premium-blue)' : st.status === 'active' ? '#ef4444' : isDark ? 'var(--border-color)' : '#e2e8f0',
                ...
                <div className="glass-card" style={{
                    ...
                    <div style={{ padding: '20px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {aiStats.lossAnalysis.contributingFactors.map((factor, i) => (
                                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #f1f5f9' }}>
                                        <span style={{ fontSize: '0.75rem', color: isDark ? 'var(--text-muted)' : '#334155', fontWeight: 600 }}>{factor.label}</span>
                                        <span style={{
                                            fontSize: '0.65rem',
                                            fontWeight: 800,
                                            color: factor.impact === 'High' ? '#ef4444' : factor.impact === 'Medium' ? '#f59e0b' : '#3b82f6',
                                            background: factor.impact === 'High' ? isDark ? 'rgba(255, 255, 255, 0.03)' : '#fef2f2' : factor.impact === 'Medium' ? '#fffbeb' : isDark ? 'rgba(255, 255, 255, 0.03)' : '#eff6ff',
                                            padding: '2px 8px',
                                            borderRadius: '4px'
                        ...
                        <div style={{ marginBottom: '20px', padding: '14px', background: 'rgba(79, 70, 229, 0.03)', borderRadius: '12px', border: '1px solid rgba(79, 70, 229, 0.1)' }}>
                            <div style={{ fontSize: '0.65rem', color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                                <button
                                    style={{ padding: '6px 10px', borderRadius: '6px', border: '1px solid #e2e8f0', background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', cursor: 'pointer', fontSize: '0.7rem' }}
                ...
                {expandedSections.includes('ai') && (
                    <div style={{ padding: '20px' }}>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <div style={{ background: 'rgba(79, 70, 229, 0.05)', borderRadius: '12px', padding: '16px', border: '1px solid rgba(79, 70, 229, 0.1)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                                    <div style={{ fontSize: '0.85rem', fontWeight: 900, color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <i className="fas fa-robot"></i> {contact.customFields?.nurtureState?.replace('_', ' ') || 'INITIALIZING'}
                                    </div>
                                    <div style={{ fontSize: '0.7rem', fontWeight: 800, color: isDark ? 'var(--text-muted)' : '#64748b', background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', padding: '2px 8px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                                    ...
                                    {['LEAD_CREATED', 'WA_SENT', 'CALL_QUEUED', 'EMAIL_SENT', 'VISIT_BOOKED', 'HANDOFF'].map((s, idx) => {
                                        ...
                                        return (
                                            <div key={s} style={{ 
                                                ...
                                                height: '6px', 
                                                borderRadius: '3px',
                                                background: isActive ? isDark ? 'var(--bg-card)' : 'var(--premium-blue)' : isCompleted ? '#818cf8' : isDark ? 'var(--border-color)' : '#e2e8f0',
                                ...
                                <div style={{ fontSize: '0.75rem', color: isDark ? 'var(--text-muted)' : '#475569', lineHeight: '1.5', fontWeight: 600, wordBreak: 'break-word' }}>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <div style={{ background: 'rgba(79, 70, 229, 0.05)', borderRadius: '12px', padding: '16px', border: '1px solid rgba(79, 70, 229, 0.1)' }}>
                                <div style={{ fontSize: '0.8rem', fontWeight: 800, color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
                            ...
                            <div style={{ background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', padding: '14px', borderRadius: '12px', border: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                                <div>
                            ...
                            <div style={{ background: 'rgba(79, 70, 229, 0.03)', padding: '14px', borderRadius: '12px', border: '1px solid rgba(79, 70, 229, 0.1)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                    <div>
                                        <div style={{ fontSize: '0.65rem', color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Deal Probability</div>
                                        <div style={{ fontSize: '1.2rem', fontWeight: 900, color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)' }}>{aiStats.dealProbability.score}% <i className={`fas fa-arrow-${aiStats.dealProbability.trend}`} style={{ fontSize: '0.8rem' }}></i></div>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ background: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', color: '#fff', fontSize: '0.55rem', padding: '2px 8px', borderRadius: '4px', fontWeight: 900 }}>AI OPTIMIZED</div>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                    {aiStats.dealProbability.factors.map((f, i) => (
                                        <span key={i} style={{ fontSize: '0.65rem', background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', padding: '3px 8px', borderRadius: '6px', border: '1px solid rgba(79, 70, 229, 0.1)', color: '#4b5563', fontWeight: 600 }}>{f}</span>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div style={{ background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', padding: '14px', borderRadius: '12px', border: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    ...
                                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: isDark ? 'var(--text-muted)' : '#64748b' }}>{aiStats.riskLevel.reason}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ background: 'linear-gradient(135deg, #4f46e5, #4338ca)', borderRadius: '16px', padding: '20px', color: '#fff', boxShadow: '0 10px 20px rgba(79, 70, 229, 0.3)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button
                                    style={{ flex: 1, padding: '10px', fontSize: '0.75rem', fontWeight: 900, background: isDark ? 'rgba(255, 255, 255, 0.03)' : '#fff', color: isDark ? 'var(--bg-card)' : 'var(--premium-blue)', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
