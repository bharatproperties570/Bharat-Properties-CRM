        ...
        <div className="glass-card" style={{ padding: '24px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 900, color: isDark ? 'var(--text-main)' : '#0f172a', letterSpacing: '-0.3px' }}>Cost Sheet</h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '12px', border: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <span style={{ fontSize: '0.95rem', fontWeight: 800, color: isDark ? 'var(--text-main)' : '#0f172a' }}>{formatCurrency(financials.dealValue)}</span>
                        </div>
                        ...
                        {(financials.valuationData?.transactionType || deal.transactionType) !== 'Full White' && (
                            <div style={{ borderTop: '1px dashed #e2e8f0', paddingTop: '8px', marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem' }}>
                                    ...
                                    <span style={{ color: isDark ? 'var(--text-main)' : '#0f172a', fontWeight: 700 }}>{formatCurrency(financials.valuationData?.stampDutyBase || financials.applicableValue)}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem' }}>
                    ...
                    <div style={{ padding: '16px', background: 'rgba(248, 250, 252, 0.3)', borderRadius: '16px', border: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                            ...
                            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: isDark ? 'var(--text-main)' : '#0f172a' }}>{formatCurrency(financials.totalGovtCharges)}</span>
                        </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(248, 250, 252, 0.5)', borderRadius: '12px', border: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid #f1f5f9' }}>
                        ...
                        <span style={{ fontSize: '0.95rem', fontWeight: 800, color: isDark ? 'var(--text-main)' : '#0f172a' }}>{formatCurrency(financials.brokerageAmount)}</span>
                    </div>
...
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: '0.85rem', color: isDark ? 'var(--text-main)' : '#475569', fontWeight: 700 }}>{formatCurrency(value)}</span>
    </div>
    );
};
