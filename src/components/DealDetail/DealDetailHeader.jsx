...
const DealDetailHeader = ({
    ...
    const getCallOptions = () => {
        ...
        if (owner && owner.phones && Array.isArray(owner.phones)) {
            owner.phones.forEach((p, idx) => {
                if (p.number) {
                    options.push({
                        label: `Owner (${p.type || 'Mobile'})`,
                        mobile: p.number,
                        contactObj: { ...owner, mobile: p.number }
                    });
                }
            });
        }
        ...
        if (associate && associate.phones && Array.isArray(associate.phones)) {
            associate.phones.forEach((p, idx) => {
                if (p.number) {
                    options.push({
                        label: `Associate (${p.type || 'Mobile'})`,
                        mobile: p.number,
                        contactObj: { ...associate, mobile: p.number }
                    });
                }
            });
        }
