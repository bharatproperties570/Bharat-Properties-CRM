...
const DealFormBuilder = ({ form, onSave, onCancel }) => {
    ...
    const handleSave = async () => {
        try {
            if (formData._id) {
                await api.put(`/deal-forms/${formData._id}`, formData);
            } else {
                await api.post('/deal-forms', formData);
            }
        ...
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#f8fafc' }}>
            ...
            <div style={{ background: '#fff', padding: '16px 40px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    <button
                        onClick={handleSave}
                        style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '10px 24px', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 15px -3px rgba(59, 130, 246, 0.4)' }}
                    >
                        <LucideSave size={18} />
                        Save Form
                    </button>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        <div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#f1f5f9', padding: '8px', borderRadius: '8px' }}>
                                <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>/public/deal/</span>
                                <input
                                    type="text"
                                    value={formData.slug}
                ...
                <div style={{ flex: 1, padding: '40px', overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ width: '100%', maxWidth: '700px', background: '#fff', borderRadius: '24px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '40px', textAlign: 'center', borderBottom: '1px solid #f1f5f9' }}>
                            ...
                            <p style={{ margin: '12px 0 0', color: '#64748b' }}>Please fill out the deal details below.</p>
                        </div>

                        {/* Sections & Fields */}
                        <div style={{ padding: '40px' }}>
                            {formData.sections.map(section => (
                                <div key={section.id} style={{ marginBottom: '40px', border: activeSectionId === section.id ? '2px solid #3b82f6' : '1px transparent', borderRadius: '16px', padding: '4px' }} onClick={() => setActiveSectionId(section.id)}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                        {section.fields.map(field => (
                                            <div
                                                ...
                                                style={{
                                                    padding: '20px',
                                                    background: selectedFieldId === field.id ? '#eff6ff' : '#fff',
                                                    border: `2px solid ${selectedFieldId === field.id ? '#3b82f6' : '#f1f5f9'}`,
                                                    borderRadius: '12px',
                                                    position: 'relative',
                                                    cursor: 'pointer'
                                                }}
                        ...
                        <div style={{ padding: '0 40px 40px' }}>
                            <button style={{ width: '100%', padding: '16px', borderRadius: '12px', border: 'none', background: '#3b82f6', color: '#fff', fontWeight: 900, fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 10px 15px -3px rgba(59, 130, 246, 0.4)' }}>
                                Submit Deal
                            </button>
                        </div>
                    </div>
...
export default DealFormBuilder;
