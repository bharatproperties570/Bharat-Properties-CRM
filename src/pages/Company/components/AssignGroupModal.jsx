...
function AssignGroupModal({ isOpen, onClose, selectedCompanies, onComplete }) {
    ...
    const handleAssign = async () => {
        ...
        try {
            ...
            toast.success("Companies assigned to group");
            onComplete();
            onClose();
        } catch (error) {
            ...
            <div style={modalStyle}>
                ...
                <div className="modal-body" style={{ padding: '24px' }}>
                    <p style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '15px' }}>
                        Assigning <strong>{selectedCompanies.length}</strong> companies to a specific broker network group.
                    </p>
                ...
                <div className="modal-footer" style={{ padding: '16px 24px', borderTop: '1px solid #f1f5f9', backgroundColor: '#f8fafc', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                    <button className="btn-outline" onClick={onClose} style={{ padding: '10px 20px', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
