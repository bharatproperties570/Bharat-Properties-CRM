...
const DealsFilterPanel = ({ isOpen, onClose, filters, onFilterChange, portalTarget = document.body }) => {
    const { isDark } = useTheme();
    const { masterFields = {}, propertyConfig = {}, projects: dynamicProjects = [], sizes: allSizes = [] } = usePropertyConfig();
        ...
        <div style={{ position: portalTarget === document.body ? 'fixed' : 'absolute', inset: 0, zIndex: 2000, display: 'flex', justifyContent: 'flex-end', fontFamily: "'Inter', sans-serif" }}>
            ...
            <div style={{
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    {/* 1. Deal Intent */}
                    <section>
                        <span style={styles.sectionTitle}>Deal Intent</span>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                            {['Sell', 'Rent', 'Lease'].map(intent => {
                    ...
                    {/* 2. Deal Stage */}
                    <section>
                        <span style={styles.sectionTitle}>Deal Stage</span>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                            {['Open', 'Quote', 'Negotiation', 'Booked', 'Closed'].map(stage => {
                                const isActive = (filters.stage || []).includes(stage);
                                return (
                                    <button
                                        key={stage}
                                        onClick={() => toggleFilterArray('stage', stage)}
                                        style={{
                    ...
                    {/* Price Range */}
                    <section>
                        <span style={styles.sectionTitle}>Price Range</span>
