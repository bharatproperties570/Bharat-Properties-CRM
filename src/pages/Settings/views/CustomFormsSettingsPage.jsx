        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff' }}>
            ...
            <div style={{ padding: '32px 40px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div>
                        <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>CUSTOM FORM BUILDER</h2>
                        <p style={{ margin: '8px 0 0', color: '#64748b', fontSize: '0.9rem' }}>Create any type of form for your unique business requirements.</p>
                    </div>
                </div>
                <button
                    onClick={handleCreateNew}
                    style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '12px 24px', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 15px -3px rgba(59, 130, 246, 0.4)' }}
                >
                    <LucidePlus size={20} />
                    Create Custom Form
                </button>
            ...
            <div style={{ flex: 1, padding: '40px', overflowY: 'auto', background: '#f8fafc' }}>
                {isLoading ? (
                    <div style={{ textAlign: 'center', padding: '100px' }}>
                        ...
                        <p style={{ marginTop: '16px', color: '#64748b' }}>Loading custom forms...</p>
                    </div>
                ) : forms.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '100px', background: '#fff', borderRadius: '24px', border: '2px dashed #e2e8f0' }}>
                        ...
                        <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1e293b', marginBottom: '8px' }}>No Custom Forms Yet</h3>
                        <p style={{ color: '#64748b', maxWidth: '400px', margin: '0 auto 24px' }}>Build forms for surveys, internal check-lists, specialized lead capture, and more.</p>
                        <button
                            onClick={handleCreateNew}
                            ...
                            <div key={form._id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '20px', overflow: 'hidden', transition: 'all 0.3s ease', position: 'relative' }}>
                                <div style={{ padding: '24px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                        <div style={{ fontSize: '0.7rem', fontWeight: 800, color: form.isActive ? '#3b82f6' : '#94a3b8', background: form.isActive ? '#eff6ff' : '#f1f5f9', padding: '4px 8px', borderRadius: '6px' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '12px' }}>
                                            ...
                                            <button onClick={() => window.open(`/public/form/${form.slug}`, '_blank')} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', padding: 0 }} title="Public View">
                                                <LucideEye size={18} />
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <p style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '4px' }}>{form.description || 'Custom business requirements form'}</p>
                                    ...
                                    <div style={{ marginTop: '24px', display: 'flex', gap: '8px' }}>
                                        <button onClick={() => handleEdit(form)} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: '1px solid #e2e8f0', background: '#fff', color: '#1e293b', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}>Edit Builder</button>
            ...
            {showEmbedModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}>
                    <div style={{ background: '#fff', width: '600px', borderRadius: '24px', padding: '40px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)' }}>
                        ...
                        <p style={{ color: '#64748b', fontSize: '0.9rem', marginBottom: '24px' }}>Copy this code to embed this custom form on your internal dashboard or third-party site.</p>
                        ...
                        <div style={{ marginTop: '32px', display: 'flex', gap: '16px', alignItems: 'center', padding: '20px', background: '#eff6ff', borderRadius: '16px', border: '1px solid #dbeafe' }}>
                            <div style={{ width: '40px', height: '40px', background: '#3b82f6', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                            ...
                            <button style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 700, fontSize: '0.8rem', cursor: 'not-allowed' }}>Generate</button>
                        </div>
                    </div>
                </div>
            )}
