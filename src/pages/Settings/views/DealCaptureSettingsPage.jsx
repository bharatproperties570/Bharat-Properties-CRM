    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff' }}>
            ...
            <div style={{ padding: '32px 40px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>DEAL CAPTURE FORMS</h2>
                    <p style={{ margin: '8px 0 0', color: '#64748b', fontSize: '0.9rem' }}>Build forms to capture deal info (Project, Unit, Price) via QR or Link.</p>
                </div>
                <button
            ...
            <div style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
                {isLoading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '100px', background: '#f8fafc', borderRadius: '24px', border: '2px dashed #e2e8f0' }}>
                        ...
                        <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1e293b', marginBottom: '8px' }}>No Deal Forms Yet</h3>
                        <p style={{ color: '#64748b', maxWidth: '400px', margin: '0 auto 24px' }}>Start building your first deal capture form to collect project and unit details easily.</p>
                        <button
                            onClick={handleCreateNew}
                            ...
                            <div key={form._id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '20px', overflow: 'hidden', transition: 'all 0.3s ease', position: 'relative' }}>
                                <div style={{ padding: '24px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                        <div style={{ fontSize: '0.7rem', fontWeight: 800, color: form.isActive ? '#3b82f6' : '#94a3b8', background: form.isActive ? '#3b82f615' : '#f1f5f9', padding: '4px 8px', borderRadius: '6px' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginTop: '24px', padding: '16px', background: '#f8fafc', borderRadius: '12px' }}>
                                        ...
                                        <div style={{ textAlign: 'center', borderLeft: '1px solid #e2e8f0', borderRight: '1px solid #e2e8f0' }}>
                                            <div style={{ fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>Deals</div>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}>
                    <div style={{ background: '#fff', width: '600px', borderRadius: '24px', padding: '40px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)' }}>
                        ...
                        <div style={{ marginTop: '32px', display: 'flex', gap: '16px', alignItems: 'center', padding: '20px', background: '#eff6ff', borderRadius: '16px', border: '1px solid #dbeafe' }}>
                            ...
                            <button style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }} onClick={() => window.open(`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(window.location.origin + '/public/deal/' + selectedForm?.slug)}`, '_blank')}>Download</button>
                        </div>
                    </div>
                </div>
            )}
