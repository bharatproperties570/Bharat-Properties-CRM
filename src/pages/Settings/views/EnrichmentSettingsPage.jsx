...
const EnrichmentSettingsPage = () => {
    ...
    const [classificationConfig, setClassificationConfig] = useState([
        ...
        { label: 'Low Intent', min: 0, max: 40, color: 'var(--text-muted)', key: 'low' },
    ]);
    ...
    const SectionHeader = ({ title, subtitle, icon, color = '#3b82f6' }) => (
        <div style={{ padding: '20px', borderBottom: '1px solid #f1f5f9', background: 'var(--bg-light)', display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <div>
                ...
                <p style={{ margin: '2px 0 0 0', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{subtitle}</p>
            </div>
        </div>
    );

    return (
        <div style={{ flex: 1, padding: '32px 40px', background: 'var(--bg-card)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ marginBottom: '32px' }}>
                <h1 style={{ fontSize: '1.8rem', fontWeight: 900, color: 'var(--text-main)', margin: '0 0 8px 0' }}>Prospecting & Enrichment</h1>
                <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '1rem' }}>Intelligence layer for lead intent detection and classification.</p>
            </div>
            ...
            {loading ? (
                <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}><i className="fas fa-spinner fa-spin"></i> Loading Rules...</div>
            ) : (
                <div style={{ flex: 1 }}>
                    {/* KEYWORDS TAB */}
                    {activeTab === 'keywords' && (
                        <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>System scans notes, logs, and WhatsApp for these keywords to auto-apply intelligence.</div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                        <tr style={{ background: 'var(--bg-light)', borderBottom: '1px solid var(--border-color)' }}>
                                            <th style={{ padding: '16px', textAlign: 'left', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Keyword</th>
                                            <th style={{ padding: '16px', textAlign: 'left', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Auto Tag</th>
                                            <th style={{ padding: '16px', textAlign: 'left', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Role Detect</th>
                                            <th style={{ padding: '16px', textAlign: 'left', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Intent Impact</th>
                                            <th style={{ padding: '16px', textAlign: 'center', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Status</th>
                                            <th style={{ padding: '16px', width: '80px' }}></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {rules.keywordRules.length === 0 ? (
                                            <tr><td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)' }}>No keyword rules defined yet.</td></tr>
                    ...
                    {activeTab === 'formula' && (
                        <div style={{ maxWidth: '600px' }}>
                            ...
                            <div style={{ padding: '24px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 0 }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <input
                                                    ...
                                                    style={{ width: '80px', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center', fontWeight: 800, color: '#2563eb' }}
                                                />
                                                <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: 700 }}>%</span>
                                            </div>
                                        </div>
                                        <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)' }}>Weighted impact of this parameter on the final intent score.</p>
                                    </div>
                                ))}
                                <div style={{ marginTop: '24px', padding: '16px', background: 'var(--bg-light)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <div style={{ padding: '24px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 0 }}>
                                    {classificationConfig.map((band, i) => (
                                        <div key={i} style={{ marginBottom: '24px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                    <input
                                                        ...
                                                        style={{ width: '60px', padding: '6px', borderRadius: '6px', border: '1px solid var(--border-color)', textAlign: 'center', fontWeight: 700 }}
                                                    />
                                            ...
                                            <div style={{ height: '8px', background: 'var(--bg-light)', borderRadius: '4px', position: 'relative', overflow: 'hidden' }}>
                            ...
                            <div style={{ padding: '24px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 0 }}>
                                <div style={{ marginBottom: '24px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                        <div style={{ fontWeight: 700, color: 'var(--text-main)' }}>Enable Auto-Detection</div>
                                        <input
                                            type="checkbox"
                                    ...
                                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>System will mark deals with "High Margin Opportunity" badge based on rules below.</p>
                                </div>
                                <div style={{ marginBottom: '20px' }}>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-muted)' }}>Inventory Age Threshold</label>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        <input
                                            type="number"
                                            value={marginConfig.ageThreshold}
                                            onChange={(e) => setMarginConfig({ ...marginConfig, ageThreshold: parseInt(e.target.value) || 0 })}
                                            style={{ width: '100px', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)', fontWeight: 700 }}
                                        />
                                        <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>days listed</span>
                                    </div>
                                </div>
                                <div style={{ marginBottom: '32px' }}>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-muted)' }}>Price Gap Requirement</label>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        <input
                                            type="number"
                                            ...
                                            style={{ width: '100px', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)', fontWeight: 700 }}
                                        />
                                        <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>% max gap between Budget & Quote</span>
                                    </div>
                                </div>
                                <button
                                    className="btn-primary"
            ...
            {isKeywordModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: 'var(--bg-card)', padding: '32px', borderRadius: '16px', width: '500px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
                        <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '24px' }}>{editingKeyword ? 'Edit Keyword Rule' : 'New Keyword Rule'}</h2>

                        <div style={{ marginBottom: '16px' }}>
                            <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', fontWeight: 600 }}>Keyword to detect</label>
                            <input type="text" value={keywordForm.keyword} onChange={e => setKeywordForm({ ...keywordForm, keyword: e.target.value })} placeholder="e.g. ROI" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }} />
                        </div>

                        <div style={{ marginBottom: '16px' }}>
                            <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', fontWeight: 600 }}>Auto-apply Tag</label>
                            <input type="text" value={keywordForm.autoTag} onChange={e => setKeywordForm({ ...keywordForm, autoTag: e.target.value })} placeholder="e.g. Investor" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }} />
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                            <div>
                                <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', fontWeight: 600 }}>Detect Role</label>
                                <select value={keywordForm.roleType} onChange={e => setKeywordForm({ ...keywordForm, roleType: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                            ...
                            <div>
                                ...
                                <input type="number" value={keywordForm.intentImpact} onChange={e => setKeywordForm({ ...keywordForm, intentImpact: parseInt(e.target.value) })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }} />
                            </div>
                        </div>
