    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-card)' }}>
            ...
            <div style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
                {forms.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '100px', background: 'var(--bg-light)', borderRadius: '24px' }}>
                        <h3 style={{ color: 'var(--text-main)' }}>No Feedback Surveys</h3>
                        <p style={{ color: 'var(--text-muted)' }}>Start collecting site visit feedback today.</p>
                    </div>
                ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '24px' }}>
                        {forms.map(form => (
                            <div key={form._id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '20px', padding: '24px' }}>
                                ...
                                <div style={{ marginTop: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', background: 'var(--bg-light)', padding: '12px', borderRadius: '12px' }}>
                                    <div style={{ textAlign: 'center' }}>
                                        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>SUBMISSIONS</div>
                                    ...
                                    <div style={{ textAlign: 'center', borderLeft: '1px solid var(--border-color)' }}>
                                        <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>AVG RATING</div>
