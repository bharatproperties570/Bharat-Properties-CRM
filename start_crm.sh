...
# 2. Kill any existing backend or tunnel on port 4001
...
cd backend
PORT=4001 node server.js > startup.log 2>&1 &
BACKEND_PID=$!
echo "Backend started with PID $BACKEND_PID"

# 4. Wait for Backend to be ready
echo "Waiting for backend to listen on port 4001..."
for i in {1..10}; do
...
for (( i=0; i<$MAX_ATTEMPTS; i++ )); do
    ...
    npx localtunnel --port 4001 --subdomain $SUBDOMAIN > tunnel_startup.log 2>&1 &
    TUNNEL_PID=$!
    sleep 7
...
if [ $SUCCESS -eq 0 ]; then
    ...
    npx localtunnel --port 4001 > tunnel_startup.log 2>&1 &
fi
